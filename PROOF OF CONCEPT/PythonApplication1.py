...
# -----------------------------
# GLOBAL BRAIN / STATE MANAGER - V22 FOUNDATION
# -----------------------------
class CompanionBrain:
    """One source of truth for the app's current state.

    This does not replace the older helper functions yet. It gathers their
    outputs into one snapshot so chat, buttons, voice, and future mobile/tablet
    views can all ask the same object instead of re-reading scattered state.
    """
    ...
    def refresh(self, source="manual"):
        today_state = load_today_task_state()
        today_progress = build_today_progress_snapshot(today_state)
        momentum = analyze_history()
        companion = build_companion_state()
        weekend_day = get_weekend_survival_day_name()
        energy = normalize_energy(today_state.get("energy") or "Normal")
        location = normalize_location(today_state.get("location") or "Work")
        completed, remaining = get_today_completed_remaining(today_state)

        mode = self.calculate_mode(today_state, today_progress, momentum, companion, weekend_day)
        ...
        self.snapshot = {
            ...
            "day_closed": bool(today_state.get("day_closed", False)),
    ...
    def calculate_mode(self, today_state, today_progress, momentum, companion, weekend_day):
        if bool(today_state.get("day_closed", False)):
            return "DAY CLOSED"
        if today_progress.get("all_minimums_met"):
            if today_progress.get("bonus"):
                return "BONUS ROUND"
            return "CLEAN FINISH"
        if weekend_day and int(today_progress.get("completed", 0) or 0) == 0:
            return f"{weekend_day.upper()} SURVIVAL"
        if normalize_energy(today_state.get("energy") or "Normal") == "Low":
            return "LOW ENERGY"
        if companion.get("mode") == "Urgent Recovery":
            return "RECOVERY"
        return "READY"
    ...
    def build_prompt(self, mode):
        ...
        if "SURVIVAL" in str(mode):
            return "SURVIVAL >"
        return labels.get(mode, f"{mode} >")

    def summary_text(self):
        s = self.snapshot or self.refresh("summary")
        progress = s.get("today_progress", {})
        momentum = s.get("momentum", {})
        companion = s.get("companion", {})
        remaining = s.get("remaining_tasks", [])
        ...
        lines = [
            ...
            f"Day Closed: {s.get('day_closed')}",
            "",
            "TODAY",
            f"Progress: {progress.get('completed')}/{progress.get('total')} tasks",
            f"EXP: {progress.get('xp_earned')}/{progress.get('xp_total')} XP",
            f"Minimums: {progress.get('minimums_met')}/{progress.get('total')}",
            f"Masteries: {progress.get('masteries')}/{progress.get('total')}",
            f"Completed: {', '.join(completed) if completed else 'None'}",
            f"Remaining: {', '.join(remaining) if remaining else 'None'}",
            "",
            "MOMENTUM",
            f"Current State: {momentum.get('current_state')}",
            f"Strongest Habit: {momentum.get('strongest_habit')}",
            f"Weakest Habit: {momentum.get('weakest_habit')}",
            f"Inactive Days: {momentum.get('inactive_days_recent')}",
            "",
            "COMPANION",
            f"Mode: {companion.get('mode')}",
            f"Mood: {companion.get('mood')}",
            f"Urgency: {companion.get('urgency')}",
            f"Voice Style: {companion.get('voice_style')}",
            f"Focus Task: {companion.get('focus_task')}",
            "",
...
def get_global_brain(refresh=False, source="manual"):
    global GLOBAL_BRAIN
    if GLOBAL_BRAIN is None:
        GLOBAL_BRAIN = CompanionBrain()
    elif refresh:
        GLOBAL_BRAIN.refresh(source)
    return GLOBAL_BRAIN


def refresh_global_brain(source="manual"):
    return get_global_brain(refresh=True, source=source)


def get_current_prompt_label():
    try:
        brain = get_global_brain(refresh=True, source="prompt")
        return brain.snapshot.get("prompt", "READY >")
    except Exception:
        return "READY >"


def format_global_brain_answer():
    brain = get_global_brain(refresh=True, source="chat")
    return brain.summary_text()


def pick_what_now_task_from_brain(brain_snapshot, survival_mode=False):
    """Pick the next task using the Global Brain snapshot instead of reloading scattered state."""
    state = brain_snapshot.get("today_state", {})
    momentum = brain_snapshot.get("momentum", {})
    candidates = []
    ...
    for task in DAILY_TASKS:
        ...
        duration = get_state_duration(state, canonical)
        next_duration = get_next_tier_duration(canonical, duration)
        score = score_what_now_candidate(task, state, momentum, survival_mode=survival_mode)
        candidates.append({
            "canonical": canonical,
            "display": task["display"],
            "duration": duration,
...
def build_brain_decision_line(brain_snapshot, pick=None):
    """Short explanation proving What Now is reading from the central brain."""
    progress = brain_snapshot.get("today_progress", {})
    momentum = brain_snapshot.get("momentum", {})
    pieces = [
        f"Brain Mode: {brain_snapshot.get('mode')}",
        f"Progress: {progress.get('completed')}/{progress.get('total')}",
        f"EXP: {progress.get('xp_earned')}/{progress.get('xp_total')}",
        f"Weakest: {momentum.get('weakest_habit')}",
    ]
    if pick:
        pieces.append(f"Selected: {pick.get('display')}")
    return " | ".join(pieces)


def calculate_recommendation_confidence(brain_snapshot, pick=None, survival_mode=False, bonus_mode=False):
    """How strongly the brain believes this is the correct next action.

    This is NOT the odds that the user will do it. It is the system's
    confidence that this recommendation is the right tactical move based on
    current state, momentum, weakest habit, progress, energy, and mode.
    """
    progress = brain_snapshot.get("today_progress", {}) or {}
    momentum = brain_snapshot.get("momentum", {}) or {}
    companion = brain_snapshot.get("companion", {}) or {}

    completed = int(progress.get("completed", 0) or 0)
    total = int(progress.get("total", 0) or 0)
    weakest = momentum.get("weakest_habit", "Unknown")
    inactive_days = int(momentum.get("inactive_days_recent", 0) or 0)
    urgency = companion.get("urgency", "medium")
    energy = brain_snapshot.get("energy", "Normal")

    confidence = 55

    # Clear tactical modes raise confidence because the brain has a strong rule.
    if survival_mode:
        confidence += 18
    if bonus_mode:
        confidence += 12
    if brain_snapshot.get("mode") == "RECOVERY":
        confidence += 10
    if urgency == "high":
        confidence += 8
    if inactive_days >= 2:
    ...
    # If the selected task is the weakest habit, the decision is more obvious.
    selected_canonical = normalize_task_name((pick or {}).get("canonical") or (pick or {}).get("display") or "")
    if selected_canonical and selected_canonical == weakest:
        confidence += 16

    # Higher score separation means the router had a cleaner winner.
    score = int((pick or {}).get("score", 0) or 0)
    if score >= 150:
        confidence += 10
    elif score >= 125:
        confidence += 7
    ...
    # If only one task remains, the choice is obvious.
    if total and total - completed == 1:
        confidence += 10

    # Low energy makes choices less certain unless survival/recovery rules are active.
    if energy == "Low" and not (survival_mode or brain_snapshot.get("mode") == "RECOVERY"):
        confidence -= 6

    return max(50, min(98, int(confidence)))


def get_priority_level(brain_snapshot, pick=None, survival_mode=False, bonus_mode=False):
    """Return the user-facing priority label for What Now.

    The math stays hidden so the chat feels like a tactical advisor instead
    of a dashboard. LOW/MEDIUM/HIGH/CRITICAL is enough for action.
    """
    if bonus_mode:
        return "BONUS"
    ...
    # Hard overrides. These make the label feel more human and less mechanical.
    if survival_mode or urgency == "high" or inactive_days >= 3:
        return "CRITICAL"
    if score >= 90:
        return "CRITICAL"
    if score >= 75:
...
def update_prompt_state_label():
    """Refresh the small command prompt label if the GUI has already created it."""
    try:
        prompt_state_var.set(get_current_prompt_label())
    except Exception:
        pass
...
def format_what_now_answer():
    """The command-center recommendation, now routed through GLOBAL_BRAIN.

    This is the first real payoff from the Global Brain step: What Now no longer
    assembles its own scattered state. It asks the brain for today's state,
    momentum state, companion state, prompt mode, EXP, remaining tasks, and
    bonus status, then makes the recommendation from that single snapshot.
    """
    brain_obj = refresh_global_brain("what_now")
    brain_snapshot = brain_obj.snapshot

    state = brain_snapshot.get("today_state", {})
    energy = brain_snapshot.get("energy", "Normal")
    location = brain_snapshot.get("location", "Work")
    snapshot = brain_snapshot.get("today_progress", {})
    momentum = brain_snapshot.get("momentum", {})
    weekend_day = brain_snapshot.get("weekend_day", "")
    zero_day = int(snapshot.get("completed", 0) or 0) == 0
    survival_mode = bool(weekend_day and zero_day)

    # If all minimums are done, use the brain snapshot's bonus state.
    if snapshot.get("all_minimums_met"):
        ...
        if bonus:
            ...
            bonus_pick = {"display": bonus.get("display"), "canonical": bonus.get("canonical"), "score": 130}
            priority_level = get_priority_level(brain_snapshot, bonus_pick, bonus_mode=True)
            decision_line = build_brain_decision_line(brain_snapshot, bonus_pick)
            return f"""WHAT NOW — BRAIN ROUTED
...
{decision_line}""".strip()
        ...
        decision_line = build_brain_decision_line(brain_snapshot)
        return f"WHAT NOW — BRAIN ROUTED\n\nAll minimums are complete and the bonus board is maxed. Clean closeout is allowed.\n\nDecision:\n{decision_line}"

    pick = pick_what_now_task_from_brain(brain_snapshot, survival_mode=survival_mode)
    if not pick:
        decision_line = build_brain_decision_line(brain_snapshot)
    ...
    priority_level = get_priority_level(brain_snapshot, pick, survival_mode=survival_mode)
    decision_line = build_brain_decision_line(brain_snapshot, pick)
...
Energy: {energy}
Location: {location}
Progress: {snapshot.get('completed')}/{snapshot.get('total')} tasks • {snapshot.get('xp_earned')}/{snapshot.get('xp_total')} XP
...
# CHAT COMMAND ROUTER
# -----------------------------
def is_morning_briefing_request(user_input):
    text = str(user_input or "").lower().strip()
    return any(phrase in text for phrase in [
        "morning briefing", "briefing", "daily briefing", "start briefing",
        "morning report", "daily report"
    ])


def is_insights_request(user_input):
    text = str(user_input or "").lower().strip()
    return any(phrase in text for phrase in [
        "insights", "show insights", "open insights", "momentum insights"
    ])


def is_trends_request(user_input):
    text = str(user_input or "").lower().strip()
    return any(phrase in text for phrase in [
        "trends", "show trends", "open trends", "trend detection", "trend dashboard"
    ])


def handle_backup_command(user_input):
    return backup_now(show_popup=False)


def handle_what_now_command(user_input):
    return format_what_now_answer()


def handle_max_reps_command(user_input):
    return format_max_reps_answer(user_input)


def handle_workout_log_command(user_input):
    return format_workout_log_answer()


def handle_backfill_command(user_input):
    date_key = parse_backfill_date(user_input)
    detected, duration, record = update_history_for_date_from_text(date_key, user_input)
    return format_backfill_response(date_key, detected, duration, record)


def handle_finish_day_command(user_input):
    return handle_day_complete_command(final=True)


def handle_day_review_command(user_input):
    return handle_day_complete_command(final=False)


def handle_daily_tasks_command(user_input):
    response = format_today_tasks_status(load_today_task_state())
    show_daily_tasks_window()
    return response


def handle_task_start_command(user_input):
    return handle_task_start_request(user_input)


def handle_task_log_command(user_input):
    detected, state = update_today_tasks_from_text(user_input)
    duration = detect_duration_from_text(user_input)
    logged_names = ", ".join(get_daily_task_display_name(task) for task in detected)
    duration_label = DURATION_LABELS.get(duration, f"{duration}m")
    return f"Logged: {logged_names} — {duration_label}.\n\n" + format_today_tasks_status(state, title="UPDATED DAILY TASKS")


def handle_morning_briefing_command(user_input):
    quotes = load_quotes()
    ...
    return format_morning_briefing(packet, companion_note)


def handle_insights_command(user_input):
    show_insights_window()
    return "Opened Momentum Insights."


def handle_trends_command(user_input):
    show_trends_window()
    return "Opened Trend Detection."


def is_brain_status_request(user_input):
    text = str(user_input or "").lower().strip()
    return text in {
        "brain", "state", "global state", "brain state", "nexus",
        "show brain", "show state", "companion state"
    }


def handle_brain_status_command(user_input):
    return format_global_brain_answer()

def normalize_command_text(user_input):
    """Normalize chat commands so aliases are easy to match."""
    text = str(user_input or "").lower().strip()
    text = re.sub(r"\s+", " ", text)
    return text
...
COMMAND_HELP_ROWS = [
    ...
    ("daily tasks", "Open today's task board and show current progress."),
    ("what now / next move / plan", "Recommend the best next action from today's progress."),
    ("morning briefing / briefing", "Run the command-center briefing."),
    ("workout / exercise / gym", "Open the workout tracker."),
    ("workout log", "Show today's saved workout sessions."),
    ("max reps", "Show saved max reps. Try: max reps biceps."),
    ("trends", "Open Trend Detection."),
    ("insights", "Open Momentum Insights."),
    ("alerts", "Show caution trend alerts."),
    ("backup", "Create a backup of the local JSON files."),
    ("finish day / closeout", "Start or finish the day closeout flow."),
    ("Spanish 20m / Coding 30m", "Log task progress from the chat bar."),
    ("yesterday I did Spanish 20m", "Backfill a prior day."),
]


COMMAND_ALIASES = {
    "status": "daily tasks",
    "task status": "daily tasks",
    "tasks": "daily tasks",
    "todo": "daily tasks",
    "today": "daily tasks",
    "plan": "what now",
    ...
    "warning": "alerts",
    "warnings": "alerts",
    "closeout": "done for today",
    "close out": "done for today",
    "seal day": "finish",
    "wrap": "done for today",
    "wrap up": "done for today",
    "nexus": "brain",
    "companion state": "brain",
}


COMMAND_SUGGESTIONS = [
    ("daily tasks", "today's task board"),
    ("status", "today's task board"),
    ("what now", "best next action"),
    ("plan", "best next action"),
    ("briefing", "command-center briefing"),
    ("workout", "open workout tracker"),
    ("gym", "open workout tracker"),
    ("workout log", "today's saved workouts"),
    ("max reps", "personal records"),
    ("trends", "trend dashboard"),
    ("insights", "momentum insights"),
    ("alerts", "caution alerts"),
    ("backup", "backup data"),
    ("done", "finish today"),
    ("closeout", "finish today"),
    ("commands", "show command list"),
    ("help", "show command list"),
    ("brain", "global brain snapshot"),
    ("state", "global brain snapshot"),
]


def get_command_suggestions(partial_text, limit=4):
    """Return compact suggestions for the command bar while the user types."""
    text = normalize_command_text(partial_text)
    if not text:
        return []
    matches = []
    for command, description in COMMAND_SUGGESTIONS:
        if command.startswith(text) or text in command:
            matches.append((command, description))
    return matches[:limit]


def apply_command_alias(user_input):
    """Convert short mobile/tablet aliases into existing command phrases."""
    text = normalize_command_text(user_input)
    return COMMAND_ALIASES.get(text, user_input)


def is_command_help_request(user_input):
    text = normalize_command_text(user_input)
    return text in {
        "help", "commands", "command", "command list", "show commands",
        "show help", "what commands", "what can you do", "what do you do",
        "how do i use this", "how to use this"
    }


def format_command_help_answer():
    """Compact one-screen command palette for the chat bar."""
    rows = [
        ("help / commands", "show this list"),
        ("daily tasks / status", "today's task board"),
        ("what now / plan", "best next action"),
        ("briefing", "command-center briefing"),
        ("workout / gym", "open workout tracker"),
        ("workout log", "today's saved workouts"),
        ("max reps", "personal records"),
        ("trends", "open Trend Detection"),
        ("insights", "open Momentum Insights"),
        ("alerts", "caution trend alerts"),
        ("backup", "backup local JSON files"),
        ("done / closeout", "finish today"),
        ("brain / state", "show global brain snapshot"),
    ]

    max_command_width = max(len(command) for command, _ in rows)
    lines = ["COMMAND CENTER", ""]
    for command, description in rows:
        lines.append(f"{command.ljust(max_command_width)} - {description}")

    lines.extend([
        "",
        "FAST LOGGING",
        "Spanish 20m  |  Coding 30m  |  Workout 40m",
        "Reading 10m  |  Networking 20m",
        "",
        "BACKFILL",
        "yesterday I did Spanish 20m and workout 30m",
    ])
...
def handle_command_help(user_input):
    return format_command_help_answer()


def handle_python_or_ai_fallback(user_input):
...
COMMAND_ROUTES = [
    # Order matters: specific commands should stay above broader task logging.
    ("help", is_command_help_request, handle_command_help),
    ("brain", is_brain_status_request, handle_brain_status_command),
    ("backup", is_backup_request, handle_backup_command),
    ("morning_briefing", is_morning_briefing_request, handle_morning_briefing_command),
    ("what_now", is_what_now_request, handle_what_now_command),
    ("max_reps", is_max_reps_request, handle_max_reps_command),
    ("workout_log", is_workout_log_request, handle_workout_log_command),
    ("backfill_log", is_backfill_logging_message, handle_backfill_command),
    ("finish_day", is_finish_day_request, handle_finish_day_command),
    ("day_review", is_day_complete_request, handle_day_review_command),
    ("daily_tasks", is_task_display_request, handle_daily_tasks_command),
    ("insights", is_insights_request, handle_insights_command),
    ("trends", is_trends_request, handle_trends_command),
    ("start_task", is_task_start_request, handle_task_start_command),
    ("log_task", is_task_logging_message, handle_task_log_command),
]


def route_chat_command(user_input):
    """Run the first matching chat command and return its response.

    This keeps send_message() clean and makes every future button/action easier
    to expose through the chat bar for tablet/mobile use.
    """
    routed_input = apply_command_alias(user_input)

    for command_name, detector, handler in COMMAND_ROUTES:
        try:
            if detector(routed_input):
                return command_name, handler(routed_input)
        except Exception as error:
            return command_name, f"COMMAND ERROR — {command_name}\n\n{error}"

    return "fallback", handle_python_or_ai_fallback(user_input)
...
def send_message():
    ...
    try:
        command_suggestion_var.set("")
    except Exception:
        pass
    root.update()

    command_name, response = route_chat_command(user_input)
    insert_chat_line("Companion", response, "companion")
...
def refresh_brain_preview(user_input=""):
    # Refresh the global state first so right-panel data, prompt label, chat, and voice
    # are all reading the same source of truth.
    refresh_global_brain(source=str(user_input or "preview"))
    update_prompt_state_label()
...
root = tk.Tk()
root.title("Momentum Companion Builder Branch v25 Priority Level")
...
tk.Label(
    header,
    text="Momentum Companion Builder Branch v25 Priority Level",
    font=("Segoe UI", 20, "bold"),
    bg="#111111",
    fg="#e6e6e6"
).pack(anchor="w")
...
tk.Label(
    header,
    text="Builder Branch v22: global brain/state manager, command router, command palette, aliases, backups, workout sessions, backfill, alerts, tiered EXP + bonus rounds.",
    font=("Segoe UI", 10),
    bg="#111111",
    fg="#aaaaaa"
...
prompt_state_var = tk.StringVar(value=get_current_prompt_label())
prompt_state_label = tk.Label(
    entry_frame,
    textvariable=prompt_state_var,
    font=("Consolas", 10, "bold"),
    bg="#111111",
    fg="#8bd3ff",
    anchor="w"
)
prompt_state_label.grid(row=0, column=0, sticky="w", padx=(0, 8))

user_entry = tk.Entry(entry_frame, font=("Segoe UI", 11), bg="#222222", fg="white", insertbackground="white")
user_entry.grid(row=0, column=1, sticky="ew", ipady=6)
user_entry.focus()
...
tk.Button(
    ...
    width=10
).grid(row=0, column=2, padx=(8, 0), ipady=4)
...
speaker_button.grid(row=0, column=3, padx=(8, 0), ipady=2)
...
voice_status_label.grid(row=1, column=0, columnspan=4, sticky="w", pady=(3, 0))

command_suggestion_var = tk.StringVar(value="")
command_suggestion_label = tk.Label(
    entry_frame,
    textvariable=command_suggestion_var,
    font=("Consolas", 9),
    bg="#111111",
    fg="#8bd3ff",
    anchor="w",
...
command_suggestion_label.grid(row=2, column=0, columnspan=4, sticky="ew", pady=(2, 0))


def update_command_suggestion_bar(event=None):
    text = user_entry.get()
    ...
    compact = "   |   ".join(f"{command} - {description}" for command, description in suggestions)
    command_suggestion_var.set(compact)


def accept_first_command_suggestion(event=None):
    suggestions = get_command_suggestions(user_entry.get(), limit=1)
    if not suggestions:
        return
    command, _ = suggestions[0]
    user_entry.delete(0, tk.END)
    user_entry.insert(0, command)
    update_command_suggestion_bar()
    return "break"


user_entry.bind("<KeyRelease>", update_command_suggestion_bar)
user_entry.bind("<Tab>", accept_first_command_suggestion)
